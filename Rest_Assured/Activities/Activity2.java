...
public class Activity2 {
	...
	public void addNewUserFromFile() throws IOException {
		// Import JSON file
		FileInputStream inputJSON = new FileInputStream("src/main/java/activities/userinfo.json");
	...
	public void getUserInfo() {
		...
		response.then().body("username", equalTo("madhubabu"));
		response.then().body("firstName", equalTo("Madhu"));
		response.then().body("lastName", equalTo("Babu"));
		response.then().body("email", equalTo("mschowdary.k@mail.com"));
		response.then().body("password", equalTo("Selenium123"));
		response.then().body("phone", equalTo("8328397604"));
	}
